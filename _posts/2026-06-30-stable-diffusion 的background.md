...
### 发展脉络
![发展脉络](Pasted image 20260630222702.png)
### 从自编码器，Autoencoder（AE）开始
我们不需要给图片打上标签，整个训练过程是自监督的。

![AE结构](Pasted image 20260630223231.png)
...
![过拟合示例](Pasted image 20260630223858.png)
### 第一条路线：VAE 和 DDPM
#### VAE
![VAE结构](Pasted image 20260630224224.png)
...
![VAE采样](Pasted image 20260630224353.png)
...
![VAE生成流程](Pasted image 20260630224631.png)
...
![DDPM加噪](Pasted image 20260630225701.png)既然编码是加噪声，那解码时就应该去掉噪声。DDPM的解码器也不再是一个不可解释的神经网络，而是一个能预测若干个去噪结果的神经网络。

![DDPM去噪](Pasted image 20260630225910.png)
总结一下，DDPM对VAE做了如下改动：
...
![向量离散化](Pasted image 20260630233354.png)
忽略掉实现细节，我们可以认为VQVAE能够把图像压缩成离散向量。更准确地说，VQVAE能把图像等比例压缩成离散的「小图像」。压缩成二维图像而不是一维向量，能够保留原图像的一些空间特性，为之后第二步图像生成铺路![VQVAE压缩](Pasted image 20260630233444.png)
...
![VQ正则化](Pasted image 20260630235419.png)
